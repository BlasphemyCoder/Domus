    ...
    $.each(imoveis, function (index, value) {
        if (value.estado == "Validado") {
            $(".grid").append(
                ...
                `<h7 class="text-default" id="aviso` + count + `">Não existem imagens</h7>
                ...
                `<span class="product__price highlight">` + value.precos[0] + ` €</span>
                    <a href="imovelDetailNaoLogado.html#`+index.split(" ").join("_")+`" class="action action--button btn btn-primary action--buy"><i class="fas fa-search-plus"></i><span class="action__text">Show More</span></a>
                </div>
                ...
                $.each(value.fotografias, function (index, value) {
                    if (index == 0) {
                        $("#imagens" + count).append(
                            ...
                            <img id="imagem`+count2+`" class="d-block w-100" src="`+ value + `" alt="Slide`+count+1+`">
                        </div>`);
                    }
                    else {
                        $("#imagens" + count).append(
